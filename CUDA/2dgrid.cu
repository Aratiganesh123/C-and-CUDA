...
__global__ void unique_gid_calculation_2d(int * data)
{
    ...
    int number_of_threads_per_block= blockDim.x * blockDim.y; 

	int block_offset = number_of_threads_per_block * blockIdx.y;

	int row_offset = number_of_threads_per_block * gridDim.x * blockIdx.y ;
